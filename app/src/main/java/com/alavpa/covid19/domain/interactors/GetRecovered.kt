...
class GetRecovered(private val apiRepository: ApiRepository) : Interactor<List<Case>>() {
    var slug: String = ""
    override fun build(): Single<List<Case>> {
        return apiRepository.getRecovered(slug).map { recovered ->
            recovered.groupBy { case -> case.date }.values.map { cases ->
                Case(
