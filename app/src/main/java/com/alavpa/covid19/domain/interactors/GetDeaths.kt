...
class GetDeaths(private val apiRepository: ApiRepository) : Interactor<List<Case>>() {
    var slug: String = ""
    override fun build(): Single<List<Case>> {
        return apiRepository.getDeaths(slug).map { deaths ->
            deaths.groupBy { case -> case.date }.values.map { cases ->
                Case(
